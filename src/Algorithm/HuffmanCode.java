...
//            }
            if(i+8>str.length()){
                bytesCode[byteIndex++]=Byte.parseByte(str.substring(i),2);//之前测试过了，二进制字符串分段以后这样就可以转成想要的byte
            }else{
                bytesCode[byteIndex++]=(byte) Integer.parseInt(str.substring(i,i+8),2);//终于是搞出来了，
    ...
    public void codeHuffmanTree(Node node, String code) {
        if (node.key != 0) {
            byteCodeMap.put((byte) node.key, code);//char转byte就是强制类型转换就可以了
        }
        if (node.left != null) {
