            ...
            if (insert<=ordered[0]){
                ...
                for (int j=0;j<i-1;j++){
                    if (insert<=ordered[j+1]){
                        int[] temp=Arrays.copyOfRange(ordered,0,j+1);//拷贝到某一位是不包含那一位的？是的
                        temp=Arrays.copyOf(temp,i+1);
                        temp[j+1]=insert;
                        System.arraycopy(ordered,j+1,temp,j+2,i-j-1);//这里的长度计算:从原数组已经拷贝了0~j，
...
//                从i-1到0从大到小寻找插入点，寻找的同时不断一直把前面的值后移一位，给插入元素留一个位置，由于是倒着查找的，所以只要要插入的大于等于就可以放在留的位置了
            ...
            if(index+1!=i){//这个判断的用意是有没有寻找过，比如插入元素上来就比i-1大的话，是应该放在最后的，就不做后面的插入
                arr[index+1]=insert;
            }
        }
        return arr;
