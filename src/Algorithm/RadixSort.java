...
public class RadixSort extends Sort {
    ...
    public int[] sort(int[] arr){
        ...
        int[] bucketTop=new int[10];//用来标记每次每个桶里面放了多少个
        int tens=1;
        while(bucketTop[0]!=arr.length){
            bucketTop=new int[10];//用来做退出循环条件了，所以判断之后再清空
            for (int value : arr) {//这边改为增强for循环，看着舒服了很多
                switch (value / tens % 10) {//直接对10求余得个位，除以10再对10求余得十位，以此类推
                    case 0:
                        bucket[0][bucketTop[0]++] = value;
                        break;//其实bucketTop是一个index数组哦
                    case 1:
