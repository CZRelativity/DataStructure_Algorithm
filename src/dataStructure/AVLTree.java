...
//        System.out.println(Arrays.toString(randArr));
        t.formAVLTree(error1);
        Boolean balance = t.isBalance(t.root);
    ...
    public boolean isBalance(SimpleNode root) {
        if (Math.abs(root.getLeftHeight() - root.getRightHeight()) > 1) {
            ...
            if (root.left != null && !isBalance(root.left)) {
                return false;
            }
            if (root.right != null && !isBalance(root.right)) {
                return false;
            }
            return true;
        }
    ...
    public void checkForRotate() {
        int leftHeight = root.getLeftHeight(), rightHeight = root.getRightHeight();
